...
function sumOdds(numbers) {
  // Your code here
  let sumOfOdds =0;
  numbers.forEach(number => (number%2 !== 0? sumOfOdds+= number:0));
  return sumOfOdds;
}
...
function characterCount(string, c) {
  // Your code here
  string = string.toLowerCase().split(c.toLowerCase()).length - 1;
  return string;
}
// console.log(characterCount("Hi my name is ali h h h h h h testing testing .... hello world", "z"));
...
function largestIncrement(numbers) {
  ...
  {
    difInNumbers.push(((+pre)-(+curr)));
    return curr;
  });
  difInNumbers = difInNumbers.map(number=> (number < 0? number*-1: number));
  difInNumbers.forEach(number => (number>bNum? bNum = number: 0));
  return bNum;
}
...
function afterX(numbers, x) {
  // Your code here
  let afterX_ = false;
  let result = numbers.filter(number => (number === x? afterX_ = true: afterX_))
  result.shift();
...
function elementInArray(numbers, x) {
  // Your code here
  return numbers.some(number => (x===number));
}
// console.log(elementInArray([5,8, 6, 7, 8], 6));
